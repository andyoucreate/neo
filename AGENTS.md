...
- SDK-first: wrap the Codex Agent SDK, don't replace it — SDK updates flow through naturally
...
Orchestration framework for autonomous developer agents. Wraps the Codex Agent SDK with clone isolation, 3-level recovery, DAG workflows, concurrency control, budget guards, and approval gates.
...
- @anthropic-ai/Codex-agent-sdk is the only runtime AI dependency
...
- Race condition prevention: wrap appendFile calls in withWriteLock() for atomic operations — see decisions.ts, directive-store.ts, and stores/jsonl.ts (fixed in PR#197, PR#198)
