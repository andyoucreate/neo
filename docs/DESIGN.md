...
// 3. Health check — every 5 minutes
{
  ...
  payload: {
    kind: "agentTurn",
    message: "Check: (1) Are any ACPX sessions stuck >30min? (2) Are any Notion tickets in 'In Progress' >4h without PR? (3) Disk usage >80%? (4) Any process using >90% CPU? Report issues only.",
    model: "anthropic/claude-haiku-4-5"
  },
  delivery: { mode: "announce", channel: "slack", to: "channel:C_ALERTS", bestEffort: true }
}
...
// 4. Weekly cost report — Friday 17:00
{
  ...
  payload: {
    kind: "agentTurn",
    message: "Generate weekly cost report: API tokens used per project, per pipeline, per agent type. Compare with last week. Write report to Notion 'Reports' database.",
    model: "anthropic/claude-haiku-4-5"
  },
  delivery: { mode: "announce", channel: "slack", to: "channel:C_DEV_AGENTS" }
}
...
For critical shared files, locks are persisted in **both** OpenClaw memory AND a file (survives crashes):
...
| Agent uptime | Health check results | Every 5 minutes |
...
ACPX sessions: auto-resume on crash (built-in)
...
- **Model optimization:** Use Haiku for reports/health checks, Sonnet for reviews, Opus only for dev/arch/security
...
### Phase 0: Server Setup (Week 1)
- [ ] Order OVH Advance-5, wait for delivery
- [ ] Run bootstrap script
- [ ] Configure .env with all API keys (separate per service)
...
- [ ] Set up cron jobs (scanner, health check, morning brief)
...
- [ ] Create all 7 agent definitions (.claude/agents/)
- [ ] Create Bash hooks (bwrap sandbox, readonly, protect-files, check-conflicts)
...
### Phase 3: PR Review Pipeline (Week 5-6)
- [ ] Wire GitHub PR webhook → OpenClaw → 4 ACPX review sessions
...
- [ ] GitHub Actions integration as backup trigger
- [ ] Configure GitHub bot account for agent reviews (separate from human)
- [ ] Validate on real PRs from existing projects

### Phase 4: Feature Pipeline (Week 7-9)
- [ ] Wire Notion ticket dispatch → ACPX /oneshot
- [ ] Test XS/S tickets (single session)
- [ ] Test M/L tickets (full pipeline with agent teams)
- [ ] Test fallback: sequential execution if agent teams fail
...
### Phase 5: QA Pipeline (Week 10-11)
...
- [ ] Implement E2E critical path testing
...
### Phase 6: Hardening & Polish (Week 12-14)
...
**Total: ~14 weeks** (3.5 months), solo with agent assistance.
