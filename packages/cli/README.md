...
```bash
neo supervise              # Start the daemon (detached)
neo supervise --status     # Show supervisor status
neo supervise --attach     # Open the TUI for a running supervisor
neo supervise --kill       # Stop the running supervisor
neo supervise --message "Focus on the auth module"  # Send a message to the supervisor
neo supervise --name prod-supervisor  # Use a named instance
```
...
| `--attach` | Open the TUI for a running supervisor |
| `--message` | Send a message to the supervisor inbox |

#### Supervisor Daemon

The supervisor runs as a background daemon that autonomously monitors your repositories, dispatches agents, and manages the development workflow. When you run `neo supervise`:

1. A detached daemon process starts
2. It exposes a health endpoint at `localhost:<port>/health`
3. It accepts webhooks at `localhost:<port>/webhook`
4. Logs are written to `~/.neo/supervisors/<name>/daemon.log`

#### Supervisor TUI

The TUI (Terminal User Interface) provides a live dashboard for monitoring the supervisor:

- **Header**: Shows PID, port, heartbeat count, uptime, and live status
...
```bash
# Start daemon and immediately attach TUI
neo supervise && neo supervise --attach
